  ...
  <div class="container">
    ...
    <main class="main-section">
      ...
      <section class="panel-section">
        ...
        $selectQuery = "SELECT categoryName FROM categories WHERE ID = {$id}";

        $result = $conn->query($selectQuery);

        $categoryName = $result->fetch_object()->categoryName;

        $conn->close();
        ?>


        <h2 class="panel-heading2">Edytuj kategorię</h2>
        <form action="editCategory.php" class="forms" id="edit-category-form" method="post">
          <input type="hidden" name="id" value='<?= $id ?>'>
          <div class="place-holder">
            <label for="categoryName" class="labels">Nazwa kategorii: </label>
            <input value="<?= $categoryName ?>" type="text" name="categoryName" class="inputs" required="required">
          </div>
